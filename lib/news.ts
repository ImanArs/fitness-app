export const allNews = [
  {
    id: 1,
    title: "10 Best Exercises for Building Muscle",
    summary:
      "Discover the most effective exercises to build muscle mass quickly and efficiently.",
    image:
      "https://images.ctfassets.net/8urtyqugdt2l/an332LVc89C33kZl3zNAp/cc364829dedecb7dde2ef092c289c2bc/desktop-bulgarian-split-squats.jpg",
    likes: 245,
    date: "March 5, 2025",
    content: `
      <p>Building muscle requires consistent effort both in the gym and in the kitchen. Here are the 10 best exercises that should be part of any muscle-building routine:</p>
      
      <h2>1. Squats</h2>
      <p>The king of all exercises, squats work your entire lower body and core. They also trigger a strong hormonal response that aids in overall muscle growth.</p>
      
      <h2>2. Deadlifts</h2>
      <p>Another compound movement that engages multiple muscle groups simultaneously. Deadlifts build strength in your posterior chain, including your back, glutes, and hamstrings.</p>
      
      <h2>3. Bench Press</h2>
      <p>The standard for chest development, bench presses also work your shoulders and triceps.</p>
      
      <h2>4. Pull-ups</h2>
      <p>One of the best exercises for back development, pull-ups also engage your biceps and shoulders.</p>
      
      <h2>5. Overhead Press</h2>
      <p>This exercise builds impressive shoulder strength and size while also engaging your triceps and upper chest.</p>
    `,
  },
  {
    id: 2,
    title: "Nutrition Tips for Better Recovery",
    summary:
      "Learn how proper nutrition can significantly improve your post-workout recovery time.",
    image:
      "https://i0.wp.com/www.muscleandfitness.com/wp-content/uploads/2019/07/Male-Stretching-Shadow.jpg?quality=86&strip=all",
    likes: 189,
    date: "Feb 24, 2025",
    content: `
      <p>Post-workout recovery is just as important as the workout itself. Proper nutrition plays a crucial role in muscle repair and energy replenishment.</p>

      <h2>1. Prioritize Protein Intake</h2>
      <p>Protein helps repair muscle fibers and promotes growth. Aim for high-quality sources such as chicken, fish, eggs, and plant-based proteins.</p>

      <h2>2. Don't Neglect Carbs</h2>
      <p>Carbohydrates replenish glycogen stores depleted during exercise. Whole grains, fruits, and starchy vegetables are excellent choices.</p>

      <h2>3. Stay Hydrated</h2>
      <p>Water is essential for metabolic functions, including muscle recovery. Drink enough water throughout the day, especially post-exercise.</p>

      <h2>4. Get Enough Micronutrients</h2>
      <p>Vitamins and minerals like magnesium, zinc, and vitamin C support muscle function and recovery. A balanced diet with plenty of vegetables is key.</p>

      <h2>5. Timing Matters</h2>
      <p>Consuming a meal rich in protein and carbohydrates within 30-60 minutes after your workout can optimize muscle recovery.</p>

      <p>By following these nutrition strategies, you can speed up your recovery and maximize performance in your next session.</p>
    `,
  },
  {
    id: 3,
    title: "The Science Behind HIIT Workouts",
    summary:
      "Understanding why High-Intensity Interval Training is so effective for fat loss and fitness.",
    image:
      "https://cdn.shopify.com/s/files/1/1214/7132/files/Woman_sweting_on_dumbells.jpg?v=1706282619",
    likes: 312,
    date: "22 Jan, 2025",
    content: `
      <p>High-Intensity Interval Training (HIIT) is one of the most efficient workout methods for burning fat and improving cardiovascular health. But what makes it so effective?</p>

      <h2>1. The Afterburn Effect</h2>
      <p>HIIT workouts create an 'afterburn' effect known as Excess Post-Exercise Oxygen Consumption (EPOC). This means your body continues to burn calories even after your workout is over.</p>

      <h2>2. Time Efficiency</h2>
      <p>Unlike traditional cardio, HIIT workouts typically last between 15 to 30 minutes, making them ideal for people with busy schedules.</p>

      <h2>3. Fat Loss and Muscle Preservation</h2>
      <p>Research shows that HIIT burns more fat compared to steady-state cardio, while also helping to maintain lean muscle mass.</p>

      <h2>4. Improved Heart Health</h2>
      <p>HIIT workouts push your heart rate to near its maximum, improving cardiovascular endurance and overall heart health over time.</p>

      <h2>5. No Equipment Needed</h2>
      <p>Bodyweight HIIT exercises like burpees, jump squats, and mountain climbers can be done anywhere, making this workout style highly accessible.</p>

      <p>If you're looking for a workout that maximizes fat loss and improves fitness in minimal time, HIIT might be the perfect choice for you!</p>
    `,
  },
];
